...
export function GeneralInformation() {
    ...
    const { setValues } = useFormExperiment()

    function insertContext(data: any) {
        generalInfoSchema.parse(data)
        setValues(data)
    ...
    return (
        <ExperimentLayout>
            <form onSubmit={handleSubmit(insertContext)} className="flex flex-col m-12 h-auto bg-white rounded-md px-6 py-4 border-[1px]">
                ...
                <button 
                onClick={() => navigate("../printForm")}  
                type="submit" className="font-bold text-white mb-6 border-[1px] p-2 rounded-md bg-sky-700 w-24">
                    PRINT DATA
                </button>
            </form>
        </ExperimentLayout>
